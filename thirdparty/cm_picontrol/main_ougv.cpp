...
BOOST_FOREACH(ptree::value_type &v,	pt.get_child("idunits"))
	...
	string nodestr = string("main_ougv");
	...
	if (ros::master::getTopics(topics)) { 
		for (ros::master::V_TopicInfo::iterator i = topics.begin(); i != topics.end(); i++) {
			boost::cmatch mr;
			if (boost::regex_match(i->name.c_str(), mr, boost::regex("^/hal/ugv/obstacle/(.+)/Estimate$"))) {
...
if (ros::master::getTopics(topics)) { 
	...
	if ( units_found != ounits ) {
	  ROS_FATAL("Units found %d does not match units defined in the config %d: ", units_found, ounits);
	  return 1;
	}
	...
	for (i=0; i<ounits; i++) {
		...
		top_state.push_back( n.subscribe<hal_ugv::State>(
			nameso[i].c_str(),10,boost::bind(StateCallback, i, _1))
		);
		ROS_INFO("%s subscribed!", nameso[i].c_str());

		// Add TakeOff controller		
		string takeoff_cmd = string("/hal/ugv/obstacle/") + oidunits[i] + string("/controller/Takeoff");
		srvTakeOff.push_back(n.serviceClient<hal_ugv::Takeoff>(takeoff_cmd.c_str()) );
	
		// Add Hover controller		
		string hover_cmd = string("/hal/ugv/obstacle/") + oidunits[i] + string("/controller/Hover");
		srvHover.push_back(n.serviceClient<hal_ugv::Hover>(hover_cmd.c_str()) );
	
		// Add VelocityHeight controller		
		string velheight_cmd = string("/hal/ugv/obstacle/") + oidunits[i] + string("/controller/VelocityHeight");
...
for (i=0; i<units; i++) {
		...
		// Subscribe to the states of the ugvs (requires explicit)
		top_state.push_back( n.subscribe<hal_ugv::State>(
			names[i].c_str(),10,boost::bind(StateCallback, i+ounits, _1))
		);
	...
	double ocontact[ounits*(units+ounits)*2];
	
	bool ifocontact[ounits];
...
/*
		...
		if (ready)
		{
			...
			for (i=0; i<ounits; i++) {
			if (ifocontact[i] == true){
				TempVel[0] = 0.02;
					TempVel[1] = 0;
					TempVel[2] = 0;
					TempEuler[0] = 0;
					TempEuler[1] = 0;
					TempEuler[2] = jstate[i*4+2]+3.14/100+randomtime/300;}
				else{
			...
			for (i=0; i<ounits; i++)
			{
				...
				{
					ocontact[i*(ounits+units)+j*2] = sqrt(pow((jstate[j*4+1]-jstate[i*4+1]),2)+pow((jstate[j*4]-jstate[i*4]),2));
					ocontact[i*(ounits+units)+j*2+1] = atan2((jstate[j*4+1]-jstate[i*4+1]),(jstate[j*4]-jstate[i*4]))-jstate[i*4+2];
				
					if (ocontact[i*(ounits+units)+j*2] < 0.33 && ocontact[i*(ounits+units)+j*2] > 0.1&& (abs(ocontact[i*(ounits+units)+j*2+1])<3.14/2))
					{
						ifocontact[i]=true;
