...
int main(int argc, char **argv)
{
	...
	// insert UVAs
	vector<ros::Subscriber> top_state(ounits);
	for (int i=0; i<ounits; i++) {
		...
		msgInsert.request.model_name = oidunits[i];
		msgInsert.request.model_type = "model://obstacle_create";
	...
	for (int i=0; i<ounits; i++) {
		...
		hal_ugv::SetTruth msgSetTruth;
		string name = string("/hal/ugv/obstacle/") + string(oidunits[i]) + string("/SetTruth");
		ros::ServiceClient srvSetTruth = n.serviceClient<hal_ugv::SetTruth>(name.c_str());
		
		if (arg_seed) {
			// choose random initial state
			msgSetTruth.request.state.x = gsl_rng_uniform(r)*ounits - ounits/2;
			msgSetTruth.request.state.y = gsl_rng_uniform(r)*ounits - ounits/2;
		}
		else {
			// choose circular initial state
			double alpha = (double)i*2*3.1415/(double)ounits;
			msgSetTruth.request.state.x = 5*cos(alpha);
			msgSetTruth.request.state.y = 5*sin(alpha);
			msgSetTruth.request.state.yaw = alpha+3.14/2;
			ROS_INFO("%lf,%lf",msgSetTruth.request.state.x,msgSetTruth.request.state.y);
