...
int main(int argc, char **argv)
{
	...
	for (int i=0; i<units; i++) {
		...
		if (i<5 && i != 1 || i == 6)
		{
			msgInsert.request.model_type = "model://createiarcgreen";
		}
		else
		{
			msgInsert.request.model_type = "model://createiarcred";
		}
	...
	for (int i=0; i<units; i++) {
		...
		string name = string("/hal/ugv/irobotcreate/") + string(idunits[i]) + string("/SetTruth");
		...
		if (arg_seed) {
			...
			msgSetTruth.request.state.yaw = alpha;
