...
{
	// Create services
	ROS_INFO("hal_ugv Actuation Init");
...
bool Actuation::GetControl(hal_ugv::State &state, double t, hal_ugv::Control &control)
{
	// Find the controller
	Controller* ptr;
	switch (current)
	...
	if (ptr->HasGoalBeenReached())
	{
		...
		// This fixed the hover bug, whichc forces the UAV to hover at (0,0,0)
 		hal_ugv::Hover::Request  h_req; 
    	hal_ugv::Hover::Response h_res;
