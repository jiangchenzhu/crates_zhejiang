...
// Dynamics
#include "dynamics/GroundForce.h"
...
namespace gazebo
{
	...
	class UGVOB: 
		...
		public hal::ugv::UGV,	/* Exposes UGV model       */
		...
		public hal::sensor::Orientation 	/* Exposes Orientation sensor    */
 	{
  	...
  	public:
	    ...
	    void Load(physics::ModelPtr model, sdf::ElementPtr root)
	    {
	    	...
	    	// Het
	    	std::string halName = (std::string)"/hal/ugv/obstacle/" + model->GetName();
	    	hal::ugv::UGV::Init(halName);
			ROS_INFO("UGVOB plugin init %s",halName.c_str());
			...
			dP.Configure(linkPtr, root->GetElement("groundforce"));	//the parameter defined in the model.sdf file
		...
		void GetTruth(hal_ugv::State& state)
		{
			// Get the state
		...
		void SetTruth(const hal_ugv::State& state)
		{
			// Get the state
			math::Pose pose(
		...
		double SetControl(const hal_ugv::Control &control)
		{
	...
	// Register the plugin
	GZ_REGISTER_MODEL_PLUGIN(UGVOB);
}
