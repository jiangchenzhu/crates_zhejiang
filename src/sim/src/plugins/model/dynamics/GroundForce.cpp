...
double GroundForce::Clamp(const double& val, const double& minval, const double& maxval)
{
...
bool GroundForce::Configure(physics::LinkPtr link, sdf::ElementPtr root)
{
	// Backup the link
	linkPtr = link;
...
void GroundForce::Update(double dt)
{
	// Get the orientation
...
double GroundForce::GetThrustForce()
{
	return thrust;
}

// Set the remaining energy (mAh)
void GroundForce::SetThrustForce(double val)
{
	thrust = val;
} 

// Copy in some control 
void GroundForce::SetControl(const hal_ugv::Control& control)
{
